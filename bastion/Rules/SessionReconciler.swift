...
nonisolated enum SessionReconciler {
    ...
    static func reconcile(_ session: AgentSession, against rules: RuleConfig) -> Outcome {
        ...
        //    list is the deny-all sentinel produced by an unsatisfiable merge.
        if let allowed = rules.allowedClients {
            guard !allowed.isEmpty else {
                return .revoked(reason: "Client allowlist is empty — no clients permitted")
            }
            guard let bundle = session.clientBundleId else {
                return .revoked(reason: "Session has no verified bundle id")
            }
