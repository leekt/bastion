...
nonisolated final class AuditExporter: @unchecked Sendable {
    ...
    private let installIdentifierProvider: @Sendable () -> String
    private let exportHMACProvider: @Sendable (Data) -> String

    private init() {
        self.installIdentifierProvider = { AuditLog.shared.installIdentifier() }
        self.exportHMACProvider = { AuditLog.shared.computeExportHMAC(over: $0) }
    }

    init(
        installIdentifier: @escaping @Sendable () -> String,
        computeExportHMAC: @escaping @Sendable (Data) -> String
    ) {
        self.installIdentifierProvider = installIdentifier
        self.exportHMACProvider = computeExportHMAC
    }
    ...
    private func renderSignedJSON(_ records: [AuditRequestRecord]) throws -> (Data, String) {
        ...
        let signature = exportHMACProvider(recordsData)
        let bundle = AuditExportBundle(
            exportedAt: Date(),
            installId: installIdentifierProvider(),
