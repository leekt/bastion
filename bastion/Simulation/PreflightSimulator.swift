...
nonisolated struct PreflightResult: Codable, Sendable {
    ...
    /// Non-blocking warning explaining why execution tracing was unavailable.
    ...
    var hasWarnings: Bool {
        !staticWarnings.isEmpty || traceWarning != nil
    }
    ...
    init(
        ...
        self.traceWarning = traceWarning
        self.simulatedSpendObservations = simulatedSpendObservations
    }
}

nonisolated enum PreflightTraceDiagnostics {
    static let unsupportedWarning = "Execution trace unavailable: this RPC provider does not support debug_traceCall, so Bastion could not verify actual token or native transfers from the execution trace."
    static let unsupportedRecommendation = "Use an RPC endpoint with debug_traceCall support for full transfer tracing before approving high-risk UserOperations."
}
...
nonisolated final class PreflightSimulator: Sendable {
    ...
    func simulate(
        ...
        // Unsupported tracing stays non-blocking, but is surfaced in the
        // approval panel/debug bundle so the owner knows simulation was partial.
        var traceAnalysis: TraceAnalysis? = nil
        var traceWarning: String? = nil
        ...
        if let rpcURL = chainRPCURL {
            ...
            traceWarning = traceResult.warning
        ...
        do {
            ...
            let hasWarnings = !staticWarnings.isEmpty || traceWarning != nil
            var recommendations: [String] = []
            if !staticWarnings.isEmpty {
                recommendations.append("Review the warnings above before approving")
            }
            if traceWarning != nil {
                recommendations.append(PreflightTraceDiagnostics.unsupportedRecommendation)
            }
    ...
    /// and native ETH spending. Unsupported tracing is non-blocking but reported.
    private nonisolated func traceSimulation(op: UserOperation, rpcURL: URL) async -> TraceSimulationResult {
        ...
        do {
            ...
            return TraceSimulationResult(analysis: analysis, warning: nil, observations: observations)
        } catch let rpcError as EthRPCError {
            if case .debugTraceUnsupported = rpcError {
                return TraceSimulationResult(
                    analysis: nil,
                    warning: PreflightTraceDiagnostics.unsupportedWarning,
                    observations: nil
                )
            }
            // Non-blocking: any RPC error — skip.
            return TraceSimulationResult(analysis: nil, warning: nil, observations: nil)
        } catch {
    ...
    private nonisolated func buildStaticOnlyResult(
        ...
        if staticWarnings.isEmpty {
            var recommendations = ["Configure a ZeroDev project ID to enable bundler simulation"]
            if traceWarning != nil {
                recommendations.append(PreflightTraceDiagnostics.unsupportedRecommendation)
            }
            return PreflightResult(
        ...
        var recommendations = staticWarnings
        if traceWarning != nil {
            recommendations.append(PreflightTraceDiagnostics.unsupportedRecommendation)
        }
        return PreflightResult(
    ...
    private nonisolated func buildBundlerFailure(
        ...
        let message = ProviderFailureDiagnostic.from(
            error: error,
            provider: .zeroDev,
            stage: .simulation
        ).userFacingMessage
        let aaError = extractAAError(from: message)
        let (diagnosis, diagnosedRecommendations) = diagnose(aaError: aaError, rawMessage: message)
        var recommendations = diagnosedRecommendations
        if traceWarning != nil {
            recommendations.append(PreflightTraceDiagnostics.unsupportedRecommendation)
        }
