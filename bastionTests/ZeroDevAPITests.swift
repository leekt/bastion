...
// MARK: - ZeroDev Provider Resilience Tests

private final class MockZeroDevTransport: @unchecked Sendable {
...
private actor ZeroDevRetrySleepRecorder {
...
@Suite("ZeroDev provider resilience")
struct ZeroDevProviderResilienceTests {

    @Test("ZeroDev retries transient HTTP failures")
    func retriesTransientHTTPFailures() async throws {
        let transport = MockZeroDevTransport(results: [
            .success((Data(#"{"jsonrpc":"2.0","id":1,"error":{"code":-1,"message":"busy"}}"#.utf8), Self.httpResponse(status: 503))),
            .success((Data(#"{"jsonrpc":"2.0","id":1,"result":["0x0000000071727De22E5E9d8BAf0edAc6f37da032"]}"#.utf8), Self.httpResponse(status: 200)))
        ])
        let api = ZeroDevAPI(
            projectId: "project",
            transport: { request in try await transport.data(for: request) },
            retryPolicyOverride: ZeroDevRetryPolicy(maxAttempts: 2, baseDelayNanoseconds: 0),
            sleep: { _ in }
        )

        let entryPoints = try await api.supportedEntryPoints(chainId: 11155111)

        #expect(entryPoints == ["0x0000000071727De22E5E9d8BAf0edAc6f37da032"])
        #expect(transport.requestCount == 2)
    }

    @Test("ZeroDev retry backoff stops when sleep is cancelled")
    func retryBackoffStopsWhenSleepIsCancelled() async throws {
        let transport = MockZeroDevTransport(results: [
            .success((Data(#"{"jsonrpc":"2.0","id":1,"error":{"code":-1,"message":"busy"}}"#.utf8), Self.httpResponse(status: 503))),
            .success((Data(#"{"jsonrpc":"2.0","id":1,"result":["0x0000000071727De22E5E9d8BAf0edAc6f37da032"]}"#.utf8), Self.httpResponse(status: 200)))
        ])
        let recorder = ZeroDevRetrySleepRecorder()
        let api = ZeroDevAPI(
            projectId: "project",
            transport: { request in try await transport.data(for: request) },
            retryPolicyOverride: ZeroDevRetryPolicy(maxAttempts: 2, baseDelayNanoseconds: 250),
            sleep: { delay in
        ...
        do {
            _ = try await api.supportedEntryPoints(chainId: 11155111)
            Issue.record("Expected retry sleep cancellation")
        } catch is CancellationError {
            #expect(transport.requestCount == 1)
            #expect(await recorder.snapshot() == [250])
        }
    }

    @Test("ZeroDev does not retry deterministic AA validation errors")
    func doesNotRetryAAValidationErrors() async throws {
        let transport = MockZeroDevTransport(results: [
            .success((Data(#"{"jsonrpc":"2.0","id":1,"error":{"code":-32500,"message":"AA24 invalid signature"}}"#.utf8), Self.httpResponse(status: 200)))
        ])
        let api = ZeroDevAPI(
            projectId: "project",
            transport: { request in try await transport.data(for: request) },
            retryPolicyOverride: ZeroDevRetryPolicy(maxAttempts: 3, baseDelayNanoseconds: 0),
            sleep: { _ in }
        )

        do {
            _ = try await api.estimateUserOperationGas(
                Self.makeRPCUserOperation(),
                entryPoint: EntryPointAddress.v0_7,
                chainId: 11155111
            )
            Issue.record("Expected AA validation error")
        } catch {
            ...
            #expect(diagnostic.category == .bundlerValidation)
            #expect(diagnostic.retryable == false)
            #expect(diagnostic.userFacingMessage.contains("AA24"))
        }

        #expect(transport.requestCount == 1)
    }

    @Test("Provider diagnostics classify minimum fee mismatch")
    func diagnosticsClassifyMinimumFeeMismatch() {
        let diagnostic = ProviderFailureDiagnostic.from(
            error: ZeroDevError.rpcError(-32000, "maxFeePerGas must be at least 0x123"),
            provider: .zeroDev,
            stage: .submission
        )

        #expect(diagnostic.category == .minimumFeeMismatch)
        #expect(diagnostic.retryable == false)
        #expect(diagnostic.recoverySuggestion.contains("fresh ZeroDev fee estimates"))
    }

    private static func httpResponse(status: Int) -> URLResponse {
        HTTPURLResponse(
            url: URL(string: "https://rpc.zerodev.app/api/v3/project/chain/11155111")!,
            statusCode: status,
    ...
    private static func makeRPCUserOperation() -> UserOperationRPC {
        UserOperationRPC(
