...
struct EIP712Tests {
    ...
    @Test("Decimal numeric strings hash like decimal integers, not bare hex")
    func decimalNumericStringsHashAsDecimal() {
        let types: [String: [EIP712Field]] = [
            "EIP712Domain": [],
            "Permit": [
                EIP712Field(name: "value", type: "uint256"),
        ...
        let domain = EIP712Domain(name: nil, version: nil, chainId: nil, verifyingContract: nil, salt: nil)
        let decimalStringHash = EthHashing.typedDataHash(EIP712TypedData(
            types: types,
            primaryType: "Permit",
            domain: domain,
            message: ["value": AnyCodable("50000000")]
        ))
        let integerHash = EthHashing.typedDataHash(EIP712TypedData(
            types: types,
            primaryType: "Permit",
            domain: domain,
            message: ["value": AnyCodable(50_000_000)]
        ))
        #expect(decimalStringHash == integerHash)
    }
}
