...
_Last reviewed: 2026-05-25 — refreshed after the second multi-agent security audit fix pass._
...
### Third-round storage, signing, wallet-group, and release hardening gaps
- **Status**: Mitigated in local third-round hardening (2026-05-25)
- **Severity**: High
- **Subsystem**: Keychain, Secure Enclave, Signing model, XPC & auth, Approval UI (sessions), Audit history, Release tooling
- **Audit ref**: 2026-05-25 third multi-agent audit

**Original risk.** The third audit found remaining fail-open edges around Secure Enclave key scoping, Keychain read failures, audit-log MAC handling, pending wallet-group agents, sender mismatch override, unpaired XPC signing, preflight side effects before hard policy gates, session cap/target handling, direct XPC bounds, failed install receipts, and release artifact binding.

**Resolution.** Secure Enclave keys now use the Bastion access group and data-protection Keychain, legacy key fallback is disabled for pre-release, Keychain read failures distinguish missing from unavailable data, config/state/session/audit storage failures fail closed, pause fallback is stored in Keychain, audit logs require an HMAC key and stored MAC, pending/revoked wallet-group agents cannot sign, UserOperation sender mismatches hard-deny, XPC signing requires an existing paired profile, hard/static policy gates run before sponsorship/build side effects, session caps reject invalid values, high-value confirmation aggregates USDC spend, session target scope includes trace-touched addresses, direct XPC signing has size/action bounds, failed install receipts cannot mark agents installed, MCP installs default to frozen locks, and release manifests repack the verified app before publishing the ZIP hash.

### Second-round policy, session, audit, MCP, and release hardening gaps
- **Status**: Mitigated in local second-round hardening (2026-05-25)
- **Severity**: High
- **Subsystem**: Signing model, XPC & auth, Approval UI (sessions), Audit history, Infrastructure
- **Audit ref**: 2026-05-25 second multi-agent audit

**Original risk.** The follow-up audit found that pause/lockdown denials could enter the owner-override flow, resume/unlock did not require owner authentication, overlapping active sessions used broad "any allow" semantics, session spend caps were per-request instead of cumulative, XPC could start before persisted session snapshots loaded, REST string booleans could trigger submission, and release/audit metadata paths still had fail-open or disclosure gaps.

**Resolution.** Pause/lockdown now hard-block signing and resume/unlock require biometric/passcode authentication. Persisted sessions load before XPC starts, active sessions all apply with tightest-scope semantics, and session ETH/USDC caps are recorded cumulatively for the grant window. High-value confirmations normalize USDC base units and reject empty phrases. Audit redaction covers reasons, client account addresses, and submission hashes/details, and tamper state prevents log rewrites. Wallet-group read responses redact internal key tags/rule bodies, on-chain install state only flips after a receipt transaction hash, REST booleans are type-strict, MCP UserOp actions have count/aggregate bounds, weak REST tokens are refused at startup, release manifests require notarized/stapled verification, and install verification checks the responding XPC executable path.

### EIP-712 typed data could authorize Permit2 / ERC-2612 / ERC-7702 silently
- **Status**: Mitigated in [PR5 (#35)](https://github.com/<owner>/bastion/pull/35)
- **Severity**: Informational
...
**Resolution.** `PermitClassifier` parses ERC-2612 Permit, Permit2 (Single / Batch / TransferFrom / BatchTransferFrom), and ERC-7702 `Authorization` shapes from typed data and surfaces a structured warning panel in the approval UI with parsed Spender / Amount / Token / Expires fields plus a "Lasting allowance" chip when applicable. Any classified permit/delegation shape now forces the approval popup even if the typed-data posture otherwise allows auto-signing.

### Startup, XPC allowlist, and read-only metadata fail-open paths
- **Status**: Mitigated in local security hardening (2026-05-24)
- **Severity**: Medium
- **Subsystem**: XPC & auth
- **Audit ref**: 2026-05-24 multi-agent audit

**Original risk.** The XPC listener started before config load, new XPC connections only checked team signature, and read-only methods could return default wallet metadata for unprofiled clients. A disallowed same-team client could reconnect after allowlist tightening and still query non-signing metadata; a narrow startup window could evaluate the default nil allowlist before persisted policy was loaded.

**Resolution.** Service startup loads config before starting XPC, `SigningManager` forces config load before global allowlist evaluation, new XPC connections are rejected when the global allowlist denies their verified bundle id, and `getPublicKey` / `getRules` / `getState` now require an existing client profile instead of falling back to the default wallet context.

### Config/state/audit persistence fail-open paths
- **Status**: Mitigated in local security hardening (2026-05-24)
- **Severity**: Medium
- **Subsystem**: Keychain, Audit history
- **Audit ref**: 2026-05-24 multi-agent audit

**Original risk.** Corrupt counters, Keychain write failures, corrupt config data, direct audit-event recording, and pause/lockdown persistence failures could either lose enforcement state or store sensitive audit details despite configured redaction.

**Resolution.** Corrupt rate/spend counters return max values, counter writes throw before signing, corrupt config loads into a paused fallback, config writes throw on Keychain failure, pause/lockdown writes use a fail-closed Keychain fallback if config persistence fails, and `AuditLog.record(_:)` applies the configured redaction level even for directly constructed `AuditEvent` values.

### Sessions and XPC connections survived allowlist tightening
- **Status**: Mitigated in [PR4 (#33)](https://github.com/<owner>/bastion/pull/33)
- **Severity**: Medium
...
- **Status**: Mitigated in [PR2 (#31)](https://github.com/<owner>/bastion/pull/31)
...
- **Status**: Mitigated in [PR3 (#32)](https://github.com/<owner>/bastion/pull/32)
...
**Resolution.** `MergedPolicy` (typed `MergedConstraint<T>` per field — `.unrestricted | .restricted(T) | .unsatisfiable(reason)`) is now the canonical merge value. `RuleEngine.mergeGroupRules` is a thin shim that flattens through `MergedPolicy.toRuleConfig()` for legacy validation paths, and empty target/selector sentinels now deny rather than disappear. The wallet-group settings panel surfaces `unsatisfiabilityReasons` so owners see exactly which constraints disagree instead of staring at silently-denying agents.

### ZeroDev project trust resolution was implicit and inconsistent
- **Status**: Mitigated in [PR1 (#30)](https://github.com/<owner>/bastion/pull/30)
