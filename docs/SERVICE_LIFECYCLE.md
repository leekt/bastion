...
7. Service status now exposes process identity and registration context.
   - `bastion status` reports version, registration status, config corruption, process ID, launch mode, bundle identifier/path, executable path, Mach service name, and launch-agent plist name.
   - This gives operators direct evidence when a CLI binds to a stale build or unexpected service owner.

8. Singleton service ownership is covered by deterministic tests.
   - The service lock is a standalone component and tests verify that a second owner is rejected until the first owner releases the lock.

9. UI routing decisions are covered by deterministic tests.
   - Notification clicks handled by the service process open Audit History locally.
   - Notification clicks handled by relay processes and relay app launches request the registered service over XPC.

10. UI handoff reconnect behavior is covered by deterministic tests.
    - `ServiceUIBridge` retries failed open requests until the registered service accepts or the reconnect budget is exhausted.

11. Service lifecycle diagnosis is scripted before helper-split retries.
    - `scripts/diagnose-service-lifecycle.sh` checks the embedded SMAppService plist, app-relative `BundleProgram`, executable targets, stale legacy LaunchAgent conflicts, current `launchctl` state, and CLI/XPC status.
    - The diagnostic calls out the `launchd.plist` constraint that `BundleProgram` is supported only for plists installed through `SMAppService`; a legacy plist carrying `BundleProgram` can reproduce the invalid Program/ProgramArguments / `EX_CONFIG` failure shape.

12. Live lifecycle verification now has a repeatable gate.
    - `scripts/verify-service-lifecycle-live.sh` verifies a signed stable app path, runs the embedded diagnostic, checks that `bastion status` responds from the expected executable and bundle path, fails duplicate-process ownership, asks the service to open Audit History over XPC, and exercises the LaunchServices relay path.
    - The CLI exposes `bastion open-ui <settings|auditHistory|diagnostics>` so notification/relay UI handoff can be probed through the same service `openUI` boundary from a terminal.
    - The CLI exposes `bastion notification-probe --id <probeId>` so the service can deliver a known notification, and `bastion notification-click-probe --id <probeId>` so diagnostics can exercise the same click-route handler without Notification Center UI.
    - `--require-notification-click` is retained as the compatibility flag for the notification leg, but the automated gate requires notification delivery plus terminal-triggered click-route diagnostics. Native Notification Center banner activation is optional manual OS-interaction evidence.
    - Evidence logs are written under `dist/lifecycle/` by default so fresh-install, reinstall, post-reboot, post-login, and notification-click runs can be compared.
    - `scripts/audit-service-lifecycle-evidence.sh` fails unless the required phase logs exist, completed successfully, contain no `FAIL:` lines, record the expected phase label, record the same app path, LaunchAgent domain, and TeamIdentifier across every phase, and include matched notification-click diagnostics for the notification-click phase.

## Remaining Work to Reach Production Quality

### Short-Term

- run `scripts/verify-service-lifecycle-live.sh --phase fresh-install --register` against a signed stable install before retrying the helper-owned path
- repeat `scripts/verify-service-lifecycle-live.sh` after rebuild/reinstall, reboot, and logout/login
- run `scripts/audit-service-lifecycle-evidence.sh` after the phase logs are collected
- validate service registration after reboot and login
- run `scripts/verify-service-lifecycle-live.sh --phase notification-click --require-notification-click` to capture notification delivery plus terminal-triggered click-route evidence; spot-check native Notification Center banner activation manually when needed

### Medium-Term

- move request orchestration fully out of the menu bar app surface area
- add live request recovery after service restart
